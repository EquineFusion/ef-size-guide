...
function parseSizes(rows, modelIds, errors) {
  ...
  for (const { row, values: v } of rows) {
    ...
    const widthMin = parseMm(v.width_min_mm, where, 'width_min_mm', errors, { required: false });
    ...
    sizes.push({
      ...
      width_min_mm: widthMin, // null = no lower limit
...
function parseSettings(rows, errors) {
  ...
  for (const key of ['tolerance_mm', 'fresh_trim_add_mm']) {
    ...
    if (!Number.isInteger(number) || number < 0 || number > 20) {
      errors.push(`Ark «settings»: «${key}» må være et helt antall millimeter mellom 0 og 20 (fant «${settings[key]}»).`);
    } else {
      settings[key] = number;
    }
  }
...
export function buildData(workbook, { generatedAt = new Date().toISOString() } = {}) {
  ...
  if (settings.wide_hoof_model && !activeModelIds.has(String(settings.wide_hoof_model))) {
    errors.push(`Ark «settings»: wide_hoof_model «${settings.wide_hoof_model}» finnes ikke som aktiv modell i arket «models».`);
  }
